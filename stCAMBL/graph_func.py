...
def mask_generator(adj_label, N=1):
    ...
    for i in range(0, cell_num):
        ...
        # non neighbors
        total_idx = torch.arange(0, cell_num-1, dtype=torch.float32)
    ...
    indices = torch.cat([adj_label.indices(), indices], axis=1)

    value = torch.cat([adj_label.values(), torch.zeros(len(x), dtype=torch.float32)])
...
def graph_construction(adata, n=6, dmax=50, mode='KNN'):
    ...
    adj_mask = mask_generator(adj_label_m1, 1)
...
def block_diag_sparse(*arrs):
        ...
        indices = torch.cat(list_indices, axis=1)
        values = torch.cat(list_values)
